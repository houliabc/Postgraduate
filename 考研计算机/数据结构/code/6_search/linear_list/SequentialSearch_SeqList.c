...
void PutSSTable(SSTable *table, KeyType key, ValueType value) {
    ...
    if (i != -1) {  // 如果 key 已经存在，则直接更新 value
        ...
        table->data[i].value = strdup(value);  // 在堆上动态分配一块新内存，存放副本，因此删除时，要free，否则内存没有释放，会发生泄露
        return;
    }

    // 如果查找表满了，直接返回————满了就不要插入了
...
void DeleteSStable(SSTable *table, KeyType key) {
    ...
    free(table->data[i].value);  // 因为前面是用strdup来复制了副本的，不free会内存泄露
