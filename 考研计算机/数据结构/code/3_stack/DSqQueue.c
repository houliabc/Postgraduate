...
// 尾入：插入元素 e 为 Q 的新的队尾元素
bool EnQueue_rear(SqQueue *Q, ElemType e) {
    // 满了则不能插入了
...
// 头入
bool EnQueue_front(SqQueue *Q, ElemType e) {
    // 满了则不能插入了
    if (((Q->rear + 1) % Q->capacity) == Q->front)
        return false;
    // 因为头指针已有元素了，所以要先动指针后插入
...
bool DeQueue_front(SqQueue *Q, ElemType *e) {
    // 判断是否为空
    if (QueueEmpty(Q))
        return false;
    if (e)
...
// 尾出
bool DeQueue_rear(SqQueue *Q, ElemType *e) {
    // 判断是否为空
    if (QueueEmpty(Q))
        return false;
...
int main() {
    ...
    EnQueue_rear(&queue, 10); // 右入
    EnQueue_front(&queue, 100); // 左入
    EnQueue_rear(&queue, 1000); // 右入
    EnQueue_front(&queue, 10000); // 左入

    printf("queue length = %d\n", QueueLength(&queue));
    QueueTraverse(&queue);

    DeQueue_front(&queue, NULL); // 左出 (10000 出队)
    QueueTraverse(&queue);
