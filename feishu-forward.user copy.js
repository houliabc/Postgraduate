...
(function () {
  ...
  function extractMessage(msgEl) {
    ...
    const fingerprint = sender + '|' + displayTime + '|' + content.substring(0, 200);
  ...
  function forwardViaWebhook(msg) {
    ...
    var text = formatForwardText(msg);
      ...
      GM_xmlhttpRequest({
        ...
        onload: function (res) {
          try {
            ...
            if (body.StatusCode === 0 || body.code === 0 || body.msg === 'ok') {
              resolve(body);
            } else {
              reject(new Error('API: ' + JSON.stringify(body)));
            }
          } catch (e) {
  ...
  function createStatusPanel() {
    ...
    panel.innerHTML =
      ...
      '<div class="ff-row"><span class="ff-label">已转发:</span><span id="ff-count">0</span> 条</div>' +
  ...
  function updateStatus(msg) {
    ...
    statusData.lastForwardPreview = msg.content.substring(0, 30);
  ...
  function processMessage(msgEl) {
    ...
    log('📨 转发:', msg.sender, msg.displayTime, '→', msg.content.substring(0, 40));
